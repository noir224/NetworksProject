...
class rdt_sender(object):
    ...
    def rdt_rcv(self, pkt, start_time):
        ...
        if (pkt.payload == "ACK"):
            self.state = STATE_0
            print("TIME: ", self.env.now, "SenderAPP: ACk received")
            ...
            self.channel.udt_send(self.pkt, start_time)
            print("Resending message again, ACK/NAK corrupted")
...
class rdt_receiver(object):
    ...
    def rdt_rcv(self, pkt, start_time):
        ...
        if (pkt.corrupted):
            ...
            print("Receiver: NAK SENT for pck", pkt.sequ_num)
            ...
            self.channel.udt_send(response, start_time)
            print("Receiver: ACK SENT for pkt", pkt.sequ_num)
            self.ReceiverAPP.deliver_data(pkt, start_time)



#it's incrementing the number and calcuating e2e here which is ok the calculation but not the incremention //need to know where exactly to
#print the e2e without it incrementing
#also why is it getting to 500 too early